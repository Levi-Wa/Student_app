...
class ScheduleUI:
    def __init__(self, page: ft.Page, manager):
        ...
        self.page.update()
        # Запускаем загрузку расписания с небольшой задержкой
        self.page.run_task(self.initialize_schedule)

    async def initialize_schedule(self):
        """Инициализация расписания с проверкой загрузки данных"""
        try:
            # Уменьшаем время ожидания и добавляем таймаут
            timeout = 2.0  # максимальное время ожидания в секундах
            start_time = datetime.datetime.now()
            
            while not self.manager.data.schedules:
                if (datetime.datetime.now() - start_time).total_seconds() > timeout:
                    break
                await asyncio.sleep(0.05)  # уменьшаем время ожидания между проверками
            
            # Если данные загружены, отображаем расписание
            if self.manager.data.schedules:
                await self.display_schedules()
            else:
                # Если данные не загрузились, показываем сообщение и пробуем загрузить снова
                ...
                # Пробуем загрузить расписание асинхронно
                await self.manager.load_schedule_for_group(
    ...
    def create_lessons(self, main_schedule: List[Dict], day_date: datetime.date, highlight_current_pair: bool = False):
        ...
        for pair in main_schedule:
            ...
            indicator_color = {
                "past": ft.Colors.RED,
                "current": ft.Colors.GREEN,
                "future": ft.Colors.BLUE
            }.get(status, ft.Colors.BLACK)

            lesson_card = ft.Container(
                content=ft.Row([
                    ft.Column([
                        ft.Text(time_start, size=16, weight="bold", color=ft.Colors.ON_SURFACE),
                        ft.Text(f"{discipline} {lesson_type}", size=12, color=ft.Colors.ON_SURFACE_VARIANT),
                        ft.Text(f"Ауд. {room}", size=12, color=ft.Colors.ON_SURFACE_VARIANT),
                        ft.Text(teacher, size=12, color=ft.Colors.ON_SURFACE_VARIANT)
                    ], expand=True),
                    ft.Container(
                        width=15,
                        height=15,
    ...
    def create_day_card(self, day: Dict, current_date: datetime.date, tomorrow_date: datetime.date,
        ...
        try:
            ...
            if not date_str:
                return ft.Text("Дата не указана", color=ft.Colors.ERROR)

            day_date = datetime.datetime.strptime(date_str, "%d.%m.%Y").date()
            day_week = day.get("dayWeek", "")
            color = ft.Colors.BLUE if force_blue else self.manager.utils.get_date_color(day_date, current_date, tomorrow_date)
            ...
            return ft.Text(f"Ошибка дня: {str(ex)}", color=ft.Colors.ERROR)
                            ...
                            lesson_card = ft.Card(
                                content=ft.Container(
                                    content=ft.Column([
                                        ...
                                        ft.Text(f"{time_display}", weight="bold", size=16, color=ft.Colors.ON_SURFACE),
                                        ft.Text(
