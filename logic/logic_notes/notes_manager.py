...
from logic.logic_notes.notes_notifications import NotesNotifications
...
class NotesManager:
    def __init__(self, schedule_manager, app):
        ...
        self.notifications = NotesNotifications(app)
        self.app = app
    ...
    async def check_notes_expiry(self, notify_callback):
        """Проверяет сроки действия заметок"""
        await self.notifications.check_notes_expiry(self.notes, notify_callback)