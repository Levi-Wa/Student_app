...
class GroupSelectionManager:
    ...
    def initialize(self):
        """Initialize groups data from API"""
        try:
            self.data.update_groups()
            if self.ui:
                self.ui.update_course_dropdown()
            logging.info("Groups data initialized")
        except Exception as e:
            logging.error(f"Error initializing groups: {e}")
    ...
    def get_courses(self):
        ...
        try:
            ...
            if not courses:
                logging.warning("No courses found in API response")
                return []
            return courses
        except Exception as e:
